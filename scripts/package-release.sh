...
cmake_args=(
  ...
  -DDOWNSPOUT_BUILD_CHIPPER=ON
  -DDOWNSPOUT_BUILD_SKREAM=ON
  -DDOWNSPOUT_BUILD_WORMS=ON
  -DDOWNSPOUT_BUILD_MAGNETO=ON
...
# cmake install(DIRECTORY) does not preserve execute bits; set them so the
# .so files in the release zip are loadable by hosts on Linux.
