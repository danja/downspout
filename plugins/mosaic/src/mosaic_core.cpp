...
void process(State&s,const std::array<float,kParameterCount>&p,const Transport&t,std::uint32_t n,double sr,const Pool*pool,const MidiEvent*events,std::uint32_t count,float*l,float*r)noexcept{
 if(l)std::fill_n(l,n,0);if(r)std::fill_n(r,n,0);s.statusLoaded=0;if(pool)for(const auto&sample:pool->samples)if(!sample.data.empty())++s.statusLoaded;s.statusMissing=s.statusLoaded==0;
 const int mode=iv(p,kMode);if((mode==1||mode==2)&&t.valid&&t.playing){const double start=downspout::generative::absoluteQuarter(t),qpf=std::clamp(t.bpm,1.0,999.0)/(60.0*std::max(1.0,sr));const std::int64_t step=static_cast<std::int64_t>(std::floor((start+1e-8)/0.25));if(step!=s.lastAutoStep){s.lastAutoStep=step;if(downspout::generative::randomUnit(iv(p,kSeed),step)<pv(p,kDensity))trigger(s,p,pool,60,96,sr);}}
 for(std::uint32_t e=0;e<count;++e)if((mode==0||mode==2)&&events[e].size>=3&&(events[e].data[0]&0xf0)==0x90&&events[e].data[2]>0)trigger(s,p,pool,events[e].data[1],events[e].data[2],sr);
 s.statusVoices=0;for(auto&v:s.voices){if(!v.active||!v.sample)continue;++s.statusVoices;const auto&sample=*v.sample;const std::uint32_t frames=sample.data.size()/sample.channels;for(std::uint32_t i=0;i<n&&v.active;++i){const auto index=static_cast<std::uint32_t>(std::clamp(v.position,0.0,static_cast<double>(frames-1)));const float a=sample.data[index*sample.channels],b=sample.channels>1?sample.data[index*sample.channels+1]:a;const float attackStep=1.0f/std::max(1.0,pv(p,kAttack)*sr),releaseStep=1.0f/std::max(1.0,pv(p,kRelease)*sr);if(!v.releasing)v.envelope=std::min(1.0f,v.envelope+attackStep);else v.envelope=std::max(0.0f,v.envelope-releaseStep);const float gl=std::sqrt(0.5f*(1-v.pan)),gr=std::sqrt(0.5f*(1+v.pan));if(l)l[i]+=a*v.envelope*gl*0.45f;if(r)r[i]+=b*v.envelope*gr*0.45f;v.position+=v.reverse?-v.increment:v.increment;if(v.position<v.start||v.position>=v.end)v.releasing=true;if(v.envelope<=0&&v.releasing)v.active=false;}}
 for(std::uint32_t i=0;i<n;++i){if(l)l[i]=std::tanh(l[i]);if(r)r[i]=std::tanh(r[i]);}
}
} // namespace downspout::mosaic
