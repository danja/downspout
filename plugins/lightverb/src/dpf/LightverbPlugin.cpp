...
protected:
 ...
 void initParameter(std::uint32_t index,Parameter&parameter)override{const auto&s=kParameterSpecs[index];parameter.name=s.name;parameter.symbol=s.symbol;parameter.hints=s.output?kParameterIsOutput:kParameterIsAutomatable;if(s.integer)parameter.hints|=kParameterIsInteger;if(index==kMidiEnabled||index==kResetMidi||index==kStatusSpaceMidi||index==kStatusMixMidi)parameter.hints|=kParameterIsBoolean;parameter.ranges={s.defaultValue,s.minimum,s.maximum};if(index==kDecaySeconds)parameter.unit="s";else if(index==kPreDelayMs)parameter.unit="ms";else if(index==kOutputDb)parameter.unit="dB";}
 ...
 void run(const float**inputs,float**outputs,std::uint32_t frames,const MidiEvent*events,std::uint32_t eventCount)override{std::array<MidiControlEvent,512>controls{};const auto count=std::min<std::uint32_t>(eventCount,controls.size());for(std::uint32_t i=0;i<count;++i){controls[i].frame=std::min(events[i].frame,frames?frames-1:0);controls[i].size=static_cast<std::uint8_t>(std::min<std::uint32_t>(events[i].size,3));const auto*source=events[i].size>MidiEvent::kDataSize?events[i].dataExt:events[i].data;for(std::uint32_t b=0;b<controls[i].size;++b)controls[i].data[b]=source[b];}const AudioBlock audio{inputs[0],inputs[1],outputs[0],outputs[1]};status_=process(engine_,parameters_,frames,audio,controls.data(),count);}
