...
Scale mode maps every held input note to the nearest note in the configured
key and scale, with downward resolution on exact ties. Each snapped note is a
register anchor. `Scale run` emits consecutive scale degrees, while `Triad`
and `Seventh` emit diatonic degree stacks. Multiple held notes contribute a
sorted union, so close anchors do not duplicate MIDI pitches.
...
- Stop also clears held scale-mode input and incomplete chord capture; the last
  completed chord latch remains available when playback resumes.
